...
export default class Arch extends Component {
	...
	getLocalData = () => {
		...
		let desktopEnvironment = localStorage.getItem('desktopEnvironment');
		if (desktopEnvironment !== null && desktopEnvironment !== undefined) {
			this.setState({ desktopEnvironment });
		}
	...
	shutDown = () => {
		...
		ReactGA.event({
			category: `Screen Change`,
			action: `Switched off Arch Linux`
		});
	...
	setDesktopEnvironment = (de) => {
		this.setState({ desktopEnvironment: de });
		localStorage.setItem('desktopEnvironment', de);
	};
		...
		return (
			<div className="w-screen h-screen overflow-hidden" id="monitor-screen">
				<LockScreen
					...
					desktopEnvironment={this.state.desktopEnvironment}
					setDesktopEnvironment={this.setDesktopEnvironment}
				/>
				<BootingScreen
					...
					turnOn={this.turnOn}
				/>
				{this.state.desktopEnvironment === "gnome" && (
